...
def _solve_inner_ot(a, b, M_cpu, ot_type, epsilon, tau, mass,
    ...
    if use_mirror_sinkhorn:
        return mirror_sinkhorn(
            ot_type, a, b, M_cpu, mass=mass, tau=tau, n_iter=ms_iter,
        )
    if ot_type == "balanced":
        if epsilon == 0:
...
def image_classification_test(loader, model, test_10crop=True):
    ...
    # Overall accuracy (kept for reference).
    overall = torch.sum(predict == all_label).item() / float(all_label.size()[0])

    # Per-class accuracy — the STANDARD VisDA-2017 metric is the *mean* of the
    # per-class accuracies (the validation set is heavily class-imbalanced, so
...
def train(config):
    ...
    mass = config["mass"]
    best_step = 0
    best_acc = 0.0                              # best mean-per-class accuracy
    best_per_class = np.zeros(class_num)        # its per-class breakdown
    best_overall = 0.0
    ...
    for id_iter in tqdm(range(config["num_iterations"]), total=config["num_iterations"]):
        if id_iter % config["test_interval"] == config["test_interval"] - 1:
            base_network.eval()
            temp_acc, temp_per_class, temp_overall = image_classification_test(
                dset_loaders, base_network, test_10crop=prep_config["test_10crop"]
            )
            temp_model = base_network  # nn.Sequential(base_network)
            if temp_acc > best_acc:
                best_step = id_iter
                best_acc = temp_acc                 # best mean-per-class accuracy
                best_per_class = temp_per_class     # remember its per-class vector
                best_overall = temp_overall
                best_model = temp_model
                checkpoint = {"base_network": best_model.state_dict()}
                torch.save(checkpoint, osp.join(config["output_path"], "best_model.pth"))
                print("\n##########     save the best model.    #############\n")
            # Per-class breakdown, easy to parse / paste into a table.
            pc_str = "  ".join(
                "{}={:.1f}".format(name, 100.0 * v)
                for name, v in zip(VISDA_CLASSES, temp_per_class)
            )
            log_str = (
                "iter: {:05d}, mean_per_class: {:.5f}, overall: {:.5f}\n"
                "    per_class: {}".format(id_iter, temp_acc, temp_overall, pc_str)
            )
            config["out_file"].write(log_str + "\n")
            config["out_file"].flush()
            print(log_str)
        ...
        if id_iter >= config["stop_step"]:
            ...
            # Per-class results table for the best checkpoint, paper-style:
            #   All  = overall accuracy on all 12 classes (cross-method metric)
            #   <12> = per-class precision
            #   Avg  = mean of the 12 per-class precisions
            ...
            # Machine-parseable single line (for the shell to build a cumulative
            # Run | Method | All | <12> | Avg table across methods).
            ...
            with torch.no_grad():
                for i in range(k):
                    ...
                    for j in range(k):
                        ...
                        M_sce = -torch.mm(ys_oh, torch.transpose(torch.log(pred_xt + 1e-10), 0, 1))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="replace the entropic-Sinkhorn / EMD inner OT solver with "
             "Mirror Sinkhorn (bias-free, NaN-free).  Works for all ot_type.",
    )
    parser.add_argument(
