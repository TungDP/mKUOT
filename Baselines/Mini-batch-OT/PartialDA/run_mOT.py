...
def train(args):
        ...
        for _ in range(k):
            ...
            if args.use_mirror_sinkhorn:
                ...
                if epsilon == 0:
                    ...
                    pi = ot.sinkhorn(a, b, M_cpu_norm, reg=epsilon)
            elif ot_type == "uot":
                ...
                if epsilon == 0:
                    ...
                    pi = ot.partial.entropic_partial_wasserstein(a, b, M_cpu_norm, m=adap_mass, reg=epsilon)
    ...
    if args.use_mirror_sinkhorn:
        with open(f"results/result_m{ot_type}_MS.txt", "a") as f:
            log_str = "method {}, iter: {:05d}, precision: {:.5f}\n".format(
                args.name + "_" + args.output, best_iter, best_acc
            )
            f.write(log_str)
    else:
        with open(f"results/result_m{ot_type}.txt", "a") as f:
            log_str = "method {}, iter: {:05d}, precision: {:.5f}\n".format(
                args.name + "_" + args.output, best_iter, best_acc
            )
            f.write(log_str)
