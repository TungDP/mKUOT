"""
Keypoint-guided mini-batch OT for the Color Transfer experiments (Sec. IV).

The Sec. IV core below is shared verbatim with the three domain-adaptation drivers and the
DeepGM module; this file is the NumPy-only copy that `utils.py` (color transfer) imports,
plus the one piece the unsupervised settings need on top of it:

  - Keypoint mining.  Color transfer is unsupervised (a "sample" is a colour, there are no
    labels), so Sec. IV-A's annotated pairs are not available.  As in the DeepGM protocol,
    we solve the UNGUIDED problem once on a preliminary batch and keep its highest-mass
    correspondences (`_select_keypoint_pairs_from_plan`).  A colour-transfer keypoint pair
    is (a fixed source palette colour, a fixed target palette colour): the pairs stay fixed
    for the whole run and are written into the first k slots of every mini-batch.
...
blended cost (Eq. 10) is solved on the masked feasible set.  Colours are static features
(unlike the DA/GM embeddings, nothing trains), so the guidance geometry never drifts.
"""
