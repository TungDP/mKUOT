# -*- coding: utf-8 -*-
"""
Mini-batch Keypoint-Guided Optimal Transport — Digits DA entry point.

Identical to the baseline train_digits.py except that:
  - Keypoint-guidance parameters (use_kpg, alpha, rho, kp_strategy) come from cfg.py
    and forwarded to the DigitsDA trainer.
  - The output directory name encodes KPG parameters when KPG is active.
"""
import logging
import os
...
# -----------------------------------------------------------------------
# Dataset helpers (unchanged from baseline)
# -----------------------------------------------------------------------
...
def main():
    ...
    prefix = "b" if args.use_bomb else ""
    # NOTE: the previous construction applied the trailing conditional to the WHOLE
    # f-string concatenation, so unguided runs got description == "" and wrote their
    # logs to snapshot/ itself.  kp_per_class is also part of the name now (pc tag,
    # omitted at the default 1 so existing snapshot names stay valid): without it,
    # the A6 keypoint-count sweep would overwrite one snapshot per (task, m, alpha).
    kpg_tag = f"_kpg_a{args.alpha}_{args.kp_strategy}" if args.use_kpg else ""
    if args.use_kpg and args.kp_per_class != 1:
        kpg_tag += f"_pc{args.kp_per_class}"
    description = (
        f"{prefix}{args.method}_{args.source_ds}_to_{args.target_ds}"
        f"_k{args.k}_m{args.mbsize}_lr{args.lr}"
        f"_epsilon{args.epsilon}_be{args.batch_epsilon}"
        f"_mass{args.mass}_tau{args.tau}{kpg_tag}"
    )
    ...
    model_da = DigitsDA(
        ...
        kp_strategy=args.kp_strategy,
        kp_per_class=args.kp_per_class,
        rho=args.rho,
        kp_metric=args.kp_metric,
        kp_probe=args.kp_probe,
        kuot_eps=args.kuot_eps,
        kuot_iters=args.kuot_iters,
    )

    # ── Training ─────────────────────────────────────────────────────────
    model_da.source_only(source_train_dl, lr=args.lr)

    if args.use_bomb:
        model_da.fit_bomb2(
            source_train_dl, target_train_dl, target_test_dl,
